...
namespace Biblioteca_examen
{
    public partial class compras : Form
    {
        ...
        private void btn_comprar_Click(object sender, EventArgs e)
        {
            ...
            //if (auxCliente.ShowDialog() == DialogResult.OK)
            //{
            //    if (cliente.ValidarCliente(auxCliente))
            ...
            //if (!aux)
            //    if (String.Equals(item.Apellido, apellido))
            //    {
            //        MessageBox.Show("Por ser miembros de la familia Simpson obtiene un 13% de descuento", "señor/a simpson");
            //        double descuento = (total * 0.87);
            //        MessageBox.Show($"EL TOTAL ES:{total} ");
            ...
            /*foreach (cliente item in this.MiClientess)
            {
                if (String.Equals(item.Apellido, "simpson"))
                {
                    MessageBox.Show("Por ser miembros de la familia Simpson tiene un descuentos del 13%", "señor/a simpson");
                    double descuento = (total * 0.87);
                    ...
                    else if  (MessageBox.Show("El total es:" + total.ToString(), "Total", MessageBoxButtons.OK, MessageBoxIcon.Question) == DialogResult.OK)
                     {   {
